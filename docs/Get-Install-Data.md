...
``` bash
az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out json --query *.outputs.mainTemplateOutputs.value.siteURL.value
```
...
``` bash
az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.siteURL
```

Now we can assign individual values to environment variables, for example:

``` bash
MOODLE_SITE_URL="$(az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.siteURL.value)"
```
...
```bash
MOODLE_LOAD_BALANCER_DNS="$(az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.loadBalancerDNS.value)"
```
...
```bash
MOODLE_ADMIN_PASSWORD="$(az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.moodleAdminPassword.value)"
```
...
```bash
MOODLE_CONTROLLER_INSTANCE_IP="$(az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.controllerInstanceIP.value)"
```
...
``` bash
MOODLE_DATABASE_DNS="$(az deployment group show --resource-group $MOODLE_RG_NAME --name $MOODLE_DEPLOYMENT_NAME --out tsv --query *.outputs.mainTemplateOutputs.value.databaseDNS.value)"
```
